...
hybrid_MF_BF = exp(sum(T_Model_evidence_hybrid(:,1)) - sum(T_Model_evidence_MF_only(:,1)));
hybrid_MB_BF = exp(sum(T_Model_evidence_hybrid(:,1)) - sum(T_Model_evidence_MB_only(:,1)));
MF_MB_BF = exp(sum(T_Model_evidence_MF_only(:,1)) - sum(T_Model_evidence_MB_only(:,1)));

UN_hybrid_MF_BF = exp(sum(Un_Model_evidence_hybrid(:,1)) - sum(Un_Model_evidence_MF_only(:,1)));
UN_hybrid_MB_BF = exp(sum(Un_Model_evidence_hybrid(:,1)) - sum(Un_Model_evidence_MB_only(:,1)));
UN_MF_MB_BF = exp(sum(Un_Model_evidence_MF_only(:,1)) - sum(Un_Model_evidence_MB_only(:,1)));
...
ylabel('subject', 'FontSize', 12)
xlabel('\Delta lme', 'FontSize', 12)
text(-10,15,'MB Model', 'FontSize', 10)
